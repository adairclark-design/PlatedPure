...
def layer3_gpt4o_compile(restaurant_name: str, context: str, profiles: list, used_source: str, social_sentiment: str = "") -> dict:
    ...
    10. SERVER INTERROGATION SCRIPT: For every SAFE and UNCERTAIN dish, provide a 'server_question' string. You MUST dynamically tailor this question to the exact ingredients or preparation method of that exact dish. DO NOT use a generic boilerplate like "Does the [dish] contain MSG?". Instead, ask about specific cross-contamination risks or ambiguous loopholes. Example for a Burger: "Are the patties cooked on the same grill space as the MSG-marinated chicken?" Example for a dish with Natural Flavors: "This contains 'Natural Flavors' — can you check the kitchen label to see if it specifically lists Yeast Extract or Hydrolyzed Soy Protein?" For UNSAFE items, output the exact string "None".
